...
Cuboid Cuboid::find_cuboid(const std::shared_ptr<CuboidSchedulerBase> & cuboid_scheduler, const std::set<std::shared_ptr<TblColRef>>& dimensions) {
...
Cuboid Cuboid::find_by_bytes_id(const std::shared_ptr<CuboidSchedulerBase> & cuboid_scheduler, const std::vector<unsigned char>& cuboid_id) {
...
uint64_t Cuboid::get_base_cuboid_id(const std::shared_ptr<CubeDesc> & cube) { return cube->get_row_key()->get_full_mask(); }
...
uint64_t Cuboid::to_cuboid_id(const std::shared_ptr<CubeDesc> & cube_desc, const std::set<std::shared_ptr<TblColRef>>& dimensions) {
